...
import { api } from '../api';
import { Settings as SettingsIcon, Key, Download, Upload, ArrowUpDown, Copy, Check, RefreshCw, GripVertical, X, History, RotateCcw, Trash2, Tags, Plus, Edit2 } from 'lucide-react';
            ...
            <div>
                {sellers.map((seller, index) => (
                    <div key={seller.id} draggable onDragStart={() => handleDragStart(index)} onDragEnter={() => handleDragEnter(index)} onDragEnd={handleDragEnd} onDragOver={e => e.preventDefault()}
                        style={{ display: 'flex', alignItems: 'center', gap: 16, padding: 16, background: 'var(--bg-primary)', borderRadius: 8, marginBottom: 8, cursor: 'grab', border: '1px solid var(--border)' }}>
                        <GripVertical size={20} color="var(--text-secondary)" />
                        <span style={{ background: 'var(--accent)', color: 'white', width: 28, height: 28, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '0.8rem' }}>{index + 1}</span>
                        <div style={{ flex: 1 }}>
                            <div style={{ fontWeight: 600 }}>{seller.name}</div>
                            <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>{seller.email}</div>
                        </div>
                        <span className="badge" style={{ background: seller.is_in_distribution ? '#10b98122' : '#ef444422', color: seller.is_in_distribution ? '#10b981' : '#ef4444' }}>
